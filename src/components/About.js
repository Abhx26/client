...
const AboutUpdateForm = () => {
  ...
  const updateProfile = async (e) => {
    ...
    const { name } = userData;
    console.log("Updating with name:", name); // Debug log
  
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_SERVER_URL}/updateProfile`,
        { name }, // Only send the name
        {
          withCredentials: true,
          headers: {
            "Content-Type": "application/json",
      ...
      if (response?.status === 200) {
        ...
        setIsEditing(false);
        setOriginalData(userData); // Update originalData after successful update
      }
    } catch (error) {
      console.error("Error updating profile:", error);
      if (error.response && error.response?.status === 422) {
        toast.error(error.response.data.error, { toastId: "ValidationError" });
      } else {
