...
const BookingForm = () => {
  ...
  const bookingForm2 = async (e, hallId, hallName) => {
    ...
    const bookedHallId = hallId;
    const bookedHallName = hallName;
    console.log(hallId,hallName);
    
    // Format the startTime and endTime to ISO format
    ...
    // Validate the formatted startTime, endTime, and eventDate
    if (
      !isValid(parseISO(formattedStartTime)) ||
      !isValid(parseISO(formattedEndTime)) ||
      !isValid(parseISO(eventDate))
    ) {
      toast.error("Invalid start time, end time, or event date format. Please check your inputs.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.message === "Booking created successfully") {
        ...
        toast.error(data.message || "Request not sent!");
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        else if (organizingClub.length >250) {
          toast.error("Description details cannot exceed 250 characters.");
          setIsLoading(false);
  ...
  const bookingForm = async (e) => {
    ...
    try {
      ...
      if (error.response && error.response.data && error.response.data.message) {
        ...
        toast.error("Unable to fetch LT, please try again later.");
      }
    } finally {
      scrollToSection(); // Ensure this function exists
      setIsLoading(false);
    }
  };
  ...
  const handleError = (error) => {
    if (error.response) {
      if (error.response.status === 422) {
        ...
        console.error(error);
        toast.error("3 occurred while creating the booking.");
      }
    } else {
      console.error(error);
      toast.error("An error 4 occurred while creating the booking.");
    }
  };
                    ...
                    {availableLTs.map((lt, index) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-center">
                          <button
                            onClick={(e) => bookingForm2(e, lt.id, lt.name)} // Pass _id and name to bookingForm2
