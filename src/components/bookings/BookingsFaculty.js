...
const BookingFaculty = () => {
  ...
  const getBookingData = async () => {
    try {
      console.log("Inside Get Booking Data");
      // Sending the GET request with proper headers
      ...
      if (response?.status === 200) {
        ...
        setBookingData(data.bookings); // Set the fetched data to the state
  
      } else {
        console.error("Status not equal to 200. Received status:", response?.status);
                ...
                <tbody>
                  ...
                  {Array.isArray(filteredBookings) && filteredBookings.length > 0 ? (
                    filteredBookings.map((booking) => (
                      // <div key={booking.id} className="my-2 ">

                      <tr key={booking.id} className="border-gray-200 text-center border-b-2  ">
  ...
  <p className="text-gray-900 whitespace-no-wrap">
    {booking.organizingClub
      ? booking.organizingClub.split(" ").slice(0, 3).join(" ") +
        (booking.organizingClub.split(" ").length > 3 ? "..." : "")
      : "N/A"}
  </p>
</td>
                        {/* <td className="px-5 py-5 text-m bg-white  border-gray-200">
                      ...
                      {booking.eventDateType === "multiple" ? 
                        ...
                        <td className="px-5 py-5 text-m bg-white  border-gray-200">
                          <button onClick={() => handleViewClick(booking.id)} className="text-m font-bold ml-5 leading-none text-gray-600 py-3 px-5 bg-gray-200 rounded hover:bg-gray-300 focus:outline-none">View</button>
                          ...
                          {/* <button
                            onClick={() => updateBooking(booking.id, "Approved By HOD")} className="text-m font-bold ml-5 leading-none text-gray-600 py-3 px-5 bg-green-200 rounded hover:bg-green-300 focus:outline-none">Approve</button> */}
                          <button
                            onClick={() => deleteBooking(booking.id)} className="text-m font-bold ml-5 leading-none text-gray-600 py-3 px-5 bg-red-200 rounded hover:bg-red-300 focus:outline-none">Cancel</button>
                        </td>
